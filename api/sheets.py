...
class DatabaseToGoogleSheetUpdater:
    ...
    def get_database_to_dataframe(self):
        with transaction.atomic():
            ...
            all_chapters = ChapterModel.objects.filter(episode=self.episode).select_related('episode')
            # all_reels = ReelModel.objects.filter(episode=self.episode).select_related('chapter', 'episode')
