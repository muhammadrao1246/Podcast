...
#     def load_sheets_from_google_sheet(self) -> dict:
#         try:    
#             temp = gsclient.open_by_url(self.sheet_url)
#             self.filename = temp.title
#             return {sheet.title: pd.DataFrame(sheet.get_all_values(), columns=sheet.get_all_values().pop(0)) for sheet in temp.worksheets() if sheet.title in ["Copy CSV Here", "Chapter Filtered"]}
#         except Exception as ex:
#             raise ValidationError("Unable to download Google Sheets file.")
            
#     def load_sheets_from_file(self) -> dict:
#         excel_file = pd.ExcelFile(self.excel_file)
#         return {sheet_name: pd.read_excel(excel_file, sheet_name=sheet_name, header=0) for sheet_name in excel_file.sheet_names if sheet_name in ["Copy CSV Here", "Chapter Filtered"]}
...
#         return str({sheet: df for sheet, df in self.all_sheets.items()})
...
class GoogleSheetProcessor:
    ...
    def load_sheets_from_bytes(self) -> dict:
        ...
        end_time = time.time()
        print(f"Load sheets from bytes time: {end_time - start_time} seconds")
        return result
